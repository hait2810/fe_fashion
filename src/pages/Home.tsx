...
export const ProductDisplay = ({ tab }: { tab: string }) => {
  const { data } = useQuery({
    queryKey: ['get_all_products', tab],
    queryFn: () => products.list({ page: 1, pageSize: 8, categoryId: tab })
  })
      ...
      {data?.data.map((item, index) => {
        return <div key={index} className="item">
          <div className="border-[1px] rounded-md border-sky-100">
            <img src={item.avatar} className="mx-auto p-2 hover:scale-95 cursor-pointer transition-all duration-500 ease-in" alt="" />
          </div>
          <div className="mt-2 px-1">
            <Link to={`/san-pham/${item._id}`}><h3 className="text-xl z-40 font-semibold text-[#0000009c] hover:text-blue-500">{item.name}</h3></Link>
            <div className="flex gap-x-4 items-center">
              <p className="font-medium text-red-500">{formatCurrency(item?.priceCurrent || 0)}</p>  <span className="text-sm text-gray-500 italic">
                {item.sold} Đã bán
              </span>
            </div>
            <div className="flex gap-x-4">
              ...
              {(item.priceOld && item.priceOld > 0) && <div className="px-2 text-white text-[12px] flex justify-center items-center bg-red-500 rounded-full">
                {Math.round((item.priceOld - (item.priceCurrent ?? 0)) / item.priceOld * 100)}%
              </div>}
            </div>
          </div>
        </div>
...
const Clothing = ({ title, category }: { title?: string, category: ICategory[] }) => {
  const [value, setValue] = useState(category[0]._id);
  ...
  return <div className="my-6 container mx-auto">
    ...
    <Tabs value={value} onChange={handleChange} centered>
      {category.map((item, index) => {
        return (
          <Tab value={item._id} key={index} label={item.name} />
        )
      })}
    </Tabs>
    ...
    <>
        ...
        <div className="container mx-auto">
          <div>
            ...
            <div className="flex flex-nowrap gap-x-16 mt-9 overflow-x-auto justify-center">
              {opt?.map((item) => {
                return <Link to={""} className="p-1">
                  <div className="flex gap-y-3 flex-col justify-center items-center flex-nowrap w-[150px]">
                    <img className="object-cover transition-transform duration-700 transform hover:scale-105" src={item.avatar} alt="" />
                    <div>
                      <h3 className="text-xl text-center">{item.name}</h3>
                      <p className="text-gray-500 text-center">63 sản phẩm</p>
                    </div>
                  </div>
                </Link>
              })}
      ...
      <section>
        {optParent?.map((item, index) => {
          const category = opt?.filter((categoryItem) => categoryItem.parentId === item._id)
          return <Clothing key={index} title={item?.name} category={category || []} />
        })}
      </section>
    </>
  );
